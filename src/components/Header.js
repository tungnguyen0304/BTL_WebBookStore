...
const AccountButton = () => {
  ...
  const handleLogOut = async () => {
    try {
      const response = await axios.post('http://localhost/BTL_Web/src/api/logout.php')
  ...
  return (
    <>
      ...
      >
        <MenuItem>
          <Avatar />{' '}
          <MenuLink onClick={() => navigate('/view-profile')}>
            Hồ sơ của bạn
          </MenuLink>
        </MenuItem>
...
const Header = () => {
  ...
  const handleSubmit = e => {
    e.preventDefault()
    if (searchTerm) navigate('/catalog-search?q=' + searchTerm)
  }
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      axios
        .get('http://localhost/BTL_Web/src/api/products.php', {
          params: {
            q: searchTerm
          }
        })
