...
const OurStore = () => {
  ...
  useEffect(() => {
    ...
    async function fetchProductFilter() {
      try {
        const response = (await axios.get("http://localhost/BTL_Web/src/api/product-info-option.php")).data;
  ...
  useEffect(() => {
    if (Object.keys(filters).length !== 0) {
      ...
      console.log(params);
      axios.get('http://localhost/BTL_Web/src/api/products.php', { params: params })
        .then(response => {
