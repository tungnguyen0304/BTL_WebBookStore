...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($ID_error_mess == 1) {
        ...
        if ($changed) {
            ...
            $qry = "UPDATE product
                    SET name = ?, unique_name = ?, categoryID = ?, image = ?, price = ?, current_qty = ?, sold_qty = ?, authorID = ?, manufacturerID = ?, description = ?
                    WHERE ID = ?";
            ...
            $qry = "UPDATE product
                    SET categoryID = ?, image = ?, price = ?, current_qty = ?, sold_qty = ?, authorID = ?, manufacturerID = ?, description = ?
                    WHERE ID = ?";
            $stmt = mysqli_prepare($conn, $qry);
